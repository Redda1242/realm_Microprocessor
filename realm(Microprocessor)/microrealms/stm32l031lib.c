...
void initSound()
{
	// Sound will be output using Timer 2 sending a square wave to PB3
	// This is alternative function for that pin (see page 46 of the datasheet)
	RCC->IOPENR |= 2; // enable GPIOB
	pinMode(GPIOB,3,2);
	GPIOB->AFR[0] = (2 << 12); // Select AF2 for this pin (Timer 2 controls it)
	RCC->APB1ENR |= 1; // enable Timer 2
	TIM2->CR1 = 0; // disable before configuring
	TIM2->CR1 |= (1 << 7); // enable ARPE
	TIM2->PSC = 15; // Prescale the 16MHz clock down to 1MHz
	TIM2->ARR = 1000; // Set initial reload value of 1000 microseconds
	TIM2->CCR2 = TIM2->ARR/2; // 50% duty;
	TIM2->CCMR1 = (1 << 15) + (1 << 14) + (1 << 13) + (1 << 11) + (1 << 10); // Output mode = PWM, Preload enable
	TIM2->CCER |= (1 << 4); // enable TIM2 CH2 output
	TIM2->EGR |= 1;
